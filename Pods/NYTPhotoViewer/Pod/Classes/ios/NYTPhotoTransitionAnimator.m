...
- (void)performZoomingAnimationWithTransitionContext:(id <UIViewControllerContextTransitioning>)transitionContext {
    ...
    BOOL isOrientationPortrait = UIInterfaceOrientationIsPortrait([[UIApplication sharedApplication] statusBarOrientation]);
    ...
    if (![NYTOperatingSystemCompatibilityUtility isiOS8OrGreater] && !isOrientationPortrait) {
        ...
        endingViewForAnimation.transform = CGAffineTransformConcat([self transformForOrientation:[[UIApplication sharedApplication] statusBarOrientation]], endingViewForAnimation.transform);
        startingViewForAnimation.transform = CGAffineTransformConcat([self transformForOrientation:[[UIApplication sharedApplication] statusBarOrientation]], startingViewForAnimation.transform);
        
        // Correct the endingView's final transform
        finalEndingViewTransform = CGAffineTransformConcat([self transformForOrientation:[[UIApplication sharedApplication] statusBarOrientation]], self.endingView.transform);
    }
    else {
