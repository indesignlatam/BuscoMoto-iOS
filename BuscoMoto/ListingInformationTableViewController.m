...
- (void)viewDidLoad {
    ...
    [_cellNext.textLabel setText:@"Continuar"];
    ...
    if(indexPath.section == 0){
        ...
        if(indexPath.row == 0){
            ...
            UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"Aceptar" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
                ...
                NSNumber *number = [formatter numberFromString:[alert.textFields firstObject].text];
                [formatter setLocale:[NSLocale currentLocale]];
                [formatter setNumberStyle:NSNumberFormatterDecimalStyle];
                [formatter setMaximumFractionDigits:0];
                
                if([alert.textFields firstObject].text.length < 6 || [alert.textFields firstObject].text.length > 9){
                    // Invalid price input
                    [_cellPrice.textLabel setTextColor:[UIColor dangerColor]];
                    [_cellPrice setAccessoryType:UITableViewCellAccessoryNone];
                    [_cellPrice.detailTextLabel setText:[formatter stringFromNumber:number]];
                    // UIALERTVIEW
            ...
            UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"Aceptar" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
                ...
                if([alert.textFields firstObject].text.length < 1 || [alert.textFields firstObject].text.length > 6){
                    // Invalid price input
                    [_cellOdometer.textLabel setTextColor:[UIColor dangerColor]];
                    [_cellOdometer setAccessoryType:UITableViewCellAccessoryNone];
                    [_cellOdometer.detailTextLabel setText:[formatter stringFromNumber:number]];
                    // UIALERTVIEW
            ...
            [[BMCOAPIManager sharedInstance]POSTListingWithParams:params onCompletion:^(Listing *listing, NSError *error){
                for (UIImage *image in _images) {
                    [[BMCOAPIManager sharedInstance]POSTListingImage:image withParams:@{@"listing_id" : listing.objectID} onCompletion:^(BOOL success, NSError *error){
                        if(success){
                            NSLog(@"SUCCESS IMAGE");
                        }else{
                            NSLog(@"FAILED IMAGE");
...
- (void)cropViewController:(TOCropViewController *)cropViewController didCropToImage:(UIImage *)image withRect:(CGRect)cropRect angle:(NSInteger)angle{
    ...
    [self.tableView reloadData];
    [cropViewController dismissAnimatedFromParentViewController:self withCroppedImage:image toFrame:_slideShow.frame completion:^{
